Oppenheimer_1968 <- function(runConfiguration){
  runConfiguration$scenarioName <- "Oppenheimer_1968_Placebo"
  runConfiguration$simulationTime <- 10100
  ...
  runConfiguration$dataSheets <- "Oppenheimer_1968"
  
  simulatedScenario <- runScenario(runConfiguration = runConfiguration)
  
  runConfiguration$scenarioName <- "Oppenheimer_1968_PB"
  ...
  plotMappingConfiguration <- enum(list(
    ...
    xOffset = list(
      "T4 control simulated" = -7200,
      "T4 PB simulated" = -7200,
      "T4 control data" = -7200,
      "T4 PB data" = -7200
    ),
  ...
  t4Mapping$addXYSeries(xValsList = observedData$Oppenheimer_1968$control[[X_VALS_COL_NAME]],
                        yValsList = observedData$Oppenheimer_1968$control[[Y_VALS_COL_NAME]],
                        yErrorList = observedData$Oppenheimer_1968$control[[Y_ERROR_COL_NAME]],
                        lables = "T4 control data")
  t4Mapping$addXYSeries(xValsList = observedData$Oppenheimer_1968$`PB treatment`[[X_VALS_COL_NAME]],
                        yValsList = observedData$Oppenheimer_1968$`PB treatment`[[Y_VALS_COL_NAME]],
                        yErrorList = observedData$Oppenheimer_1968$`PB treatment`[[Y_ERROR_COL_NAME]],
                        lables = "T4 PB data")
  t4Mapping$setPlotConfiguration(plotMappingConfiguration)
  t4Mapping$xLim <- c(0, 2)
  t4Mapping$log <- "y"
  t4Mapping$yLim <- c(0.01, 20)
  t4Mapping$xLab = "Time [days]"
  t4Mapping$yLab = "T4 PVB [ng/ml]"
  
  plotConfiguration <- enum(list(
    outputToPNG = runConfiguration$outputToPNG,
    outputName = "Oppenheimer 1968 rat",
