Reeth_1987 <- function(runConfiguration){
  runConfiguration$scenarioName <- "Reeth_1987"
  runConfiguration$simulationTime <- 240 * 60
  ...
  runConfiguration$dataSheets <- "Reeth_1987"
  ...
  t4Mapping$addXYSeries(xValsList = simulatedScenario$outputValues$data$Time / 60,
                        yValsList = rep(180, length(simulatedScenario$outputValues$data$Time)),
                        lables = "T4 baseline")
...
#  t4Mapping$yLim <- c(0, 7)
  ...
  t3Mapping$addXYSeries(xValsList = simulatedScenario$outputValues$data$Time / 60,
                        yValsList = rep(4, length(simulatedScenario$outputValues$data$Time)),
                        lables = "T3 baseline")
  ...
  plotConfiguration$outputName <- "Reeth_1987_T4"
  plotMultiPanel(t4Mapping, plotConfiguration)
  plotConfiguration$outputName <- "Reeth_1987_T3"
