Kato_2010_PB <- function(runConfiguration){
  runConfiguration$scenarioName <- "Kato_2010_Placebo"
  runConfiguration$simulationTime <- 5*24*60
  ...
  runConfiguration$dataSheets <- "Kato_2010"
  
  simulatedScenario <- runScenario(runConfiguration = runConfiguration)
  
  runConfiguration$scenarioName <- "Kato_2010_PB"
  ...
  plotMappingConfiguration <- enum(list(
    ...
    xOffset = list(
      "T4 control simulated" = -(4*24*60),
      "T4 PB simulated" = -(4*24*60),
      "T4 control data" = -(4*24*60),
      "T4 PB data" = -(4*24*60)
    ),
  ...
  t4Mapping$addXYSeries(xValsList = observedData$Kato_2010$control[[X_VALS_COL_NAME]],
                        yValsList = observedData$Kato_2010$control[[Y_VALS_COL_NAME]],
                        yErrorList = observedData$Kato_2010$control[[Y_ERROR_COL_NAME]],
                        lables = "T4 control data")
  t4Mapping$addXYSeries(xValsList = observedData$Kato_2010$`PB treatment`[[X_VALS_COL_NAME]],
                        yValsList = observedData$Kato_2010$`PB treatment`[[Y_VALS_COL_NAME]],
                        yErrorList = observedData$Kato_2010$`PB treatment`[[Y_ERROR_COL_NAME]],
                        lables = "T4 PB data")
  t4Mapping$setPlotConfiguration(plotMappingConfiguration)
  t4Mapping$xLim <- c(0, 0.2)
  t4Mapping$log <- "y"
